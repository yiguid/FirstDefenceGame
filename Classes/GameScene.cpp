...
bool GameScene::collisionWithCircle(CCPoint circlePoint, float radius, CCPoint circlePointTwo, float radiusTwo)
{
...
void GameScene::enemyGotKilled()
{
	...
	if (_enemies->count() <= 0)
	{
		...
		{
			CCLog("You win!");
			CCDirector::sharedDirector()->replaceScene(CCTransitionSplitCols::create(1, GameScene::scene()));
		}
	}
}
...
void GameScene::doGameOver()
{
	...
	{
		gameEnded = true;
		CCDirector::sharedDirector()->replaceScene(CCTransitionRotoZoom::create(1, GameScene::scene()));
	}
}
